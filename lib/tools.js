...
Tools.prototype.getTrack = function(id, callback) {
	...
	request.get({url: "https://www.deezer.com/track/"+id, headers: this.httpHeaders, jar: true}, (function(err, res, body) {
				...
				request.post({url: self.apiUrl, headers: self.httpHeaders, qs: self.apiQueries, body: "[{\"method\":\"song.getListData\",\"params\":{\"sng_ids\":[" + scopedid + "]}}]", jar: true}, (function(err, res, body) {
					if(!err && res.statusCode == 200) {
						try{
							...
							console.log("asd");
							if(json["TOKEN"]) {
								console.log("Uploaded Files are currently not supported");
								return;
							}
							var id = json["SNG_ID"];
		...
		if(!err && res.statusCode == 200 && typeof JSON.parse(_data)["DATA"] != 'undefined') {
			...
			/*console.log("TITLE: " + json["SNG_TITLE"]);
			console.log("ARTIST: " + json["ART_NAME"]);
			console.log("ALBUM: " + json["ALB_TITLE"]);
			var data = new Date(json["DIGITAL_RELEASE_DATE"]);
			console.log("ANNO: " + data.getFullYear());
			console.log("D: " + json["DISK_NUMBER"] + " T: " + json["TRACK_NUMBER"]);
			if(json["LYRICS_TEXT"] == null){
				console.log("FALSE");
			}
			console.log("LYRICS: " + json["LYRICS_TEXT"]);*/
